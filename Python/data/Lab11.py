...
def sort_students():
    ...
    students = {}
    for line in lines:
        students[line[3:].strip()] = line[0:3]
    return students

def sort_assignments_by_code():
    with open("data/assignments.txt", "r") as assignments_file:
        lines = assignments_file.readlines()
    assignments = {}
    for i in range(1, len(lines), 3):
        assignments[lines[i + 1].strip()] = lines[i + 2].strip()
    return assignments
...
def sort_assignments_by_name():
    ...
    assignments = {}
    for i in range(1, len(lines), 3):
        assignments[lines[i].strip()] = lines[i + 1].strip()
    return assignments
